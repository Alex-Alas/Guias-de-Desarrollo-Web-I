...
const imprimirPacientes = () => {
    ...
    arrayPaciente.forEach((element, index) => {
        ...
        if (btnEditar) {
            btnEditar.onclick = () => editPaciente(index);
        }
    
        if (btnEliminar) {
            btnEliminar.onclick = () => deletePaciente(index);
        }
    });
};
...
const deletePaciente = (index) => {
    ...
    if (confirm(`¿Está seguro de eliminar al paciente ${paciente[0]} ${paciente[1]}?`)) {
...
const editPaciente = (index) => {
    ...
    // Eliminar el paciente del arreglo para que se pueda agregar actualizado
    arrayPaciente.splice(index, 1);
    ...
    // Mostrar notificación
    mensaje.innerHTML = "Paciente cargado para edición. Modifique los datos y presione 'Guardar Datos'";
    toast.show();

    // Hacer focus en el nombre
    inputNombre.focus();
};
